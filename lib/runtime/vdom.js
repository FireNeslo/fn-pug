...
var VDomRuntime = function (_PugRuntime) {
  ...
  VDomRuntime.prototype.element = function element(tagName, properties) {
    if (tagName === 'ft-fixtures') console.log(properties);
    return new this.VNode(tagName, properties, properties.children);
  };

  VDomRuntime.prototype.events = function events(context, _events) {
    return new EventHook(_events, context);
  };

  VDomRuntime.prototype.handles = function handles(context, names) {
    return new HandleHook(context, names);
  };

  VDomRuntime.prototype.prop = function prop(value) {
    return new PropertyHook(value);
  };

  VDomRuntime.prototype.text = function text(_text) {
    return new this.VText(_PugRuntime.prototype.text.call(this, _text));
  };
